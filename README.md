...
| 能直接运行的工具：4 个零第三方依赖的脚本，加 2 个引擎适配脚本 | 云服务绑定（仓库内工具都不需要 API Key；若走 LLM 纠错需自备） |
...
### 用 LLM 纠错：成立，但要看错误的结构

前面「已否决的路线」全是**本地**纠错方案。**接外部 LLM** 是另一回事——两个视频实测，专名写法正确率：

| 内容类型 | 原文 | LLM 纠正后 | 上下文增量 |
|---|---|---|---|
| 英文专名 / 产品名密集 | 30.4% | **93.9%**（超过本地 Nano 的 70.5%） | **+38.2pp** |
| 中文地名密集 | 85.0% | **100.0%** | **+0.3pp** |

差别的根源不是难度，是**错误的结构**：

> **同一实体的正确写法是否在文内出现过，决定要不要喂外部上下文。**
> 文内出现过（同实体多种写法并存）→ 纯文本自洽就能收敛，不必喂上下文；
> 文内一次都没出现过（全篇一致地错）→ 必须喂，否则收益对折，而且模型会「一致地错」。

这条可以直接当决策规则用。另有两条工程细节值得单独记：`max_tokens` 小于 32K 时**返回空正文且不报错**（预算被推理 token 吃光）；`deepseek-chat` 在长文本上会原样回显、等于不做。

> ⚠️ **适用范围**：该方案为**「不考虑时间链」的方案**——输入剥离全部 `[hh:mm:ss]` 时间戳，输出同样不含时间戳。
> 它适用于「要一段读得通、专名对的全文」，但**不能直接当素材包用**（本仓库约定正文每行须带时间戳，`tools/verify_pack.py` 会判错），必须另做 cue 重新对齐才能接入流程。全部正确率数字都在此前提下测得。

完整方法、四组对照条件、时间与 token 明细见 [docs/LLM纠错层实测.md](docs/LLM纠错层实测.md)。
